...
static void clock_lost_cb (GstBus *bus, GstMessage *msg, CustomData *data) {
  __android_log_print (ANDROID_LOG_ERROR, "gst_player", "clock_lost_cb");
}

static void eos_cb (GstBus *bus, GstMessage *msg, CustomData *data) {
  JNIEnv *env = get_jni_env ();
  if(on_end_of_stream_method_id){

    (*env)->CallVoidMethod (env, data->app, on_end_of_stream_method_id);
    if ((*env)->ExceptionCheck (env)) {
      GST_ERROR ("Failed to call Java method");
      (*env)->ExceptionClear (env);
    }
    __android_log_print (ANDROID_LOG_ERROR, "gst_player", "eos_cb");
  }
...
static void error_cb (GstBus *bus, GstMessage *msg, CustomData *data) {
  ...
  if(on_error_method_id){
    JNIEnv *env = get_jni_env ();
    jstring message = (*env)->NewStringUTF(env, message_string);
    (*env)->CallVoidMethod (env, data->app, on_error_method_id, message);
    if ((*env)->ExceptionCheck (env)) {
      GST_ERROR ("Failed to call Java method");
      (*env)->ExceptionClear (env);
    }
...
static void *app_function (void *userdata) {
  ...
  __android_log_print (ANDROID_LOG_ERROR, "gst_player", "%s", data->rawPipeline);

  data->pipeline = gst_parse_launch(data->rawPipeline, &error);
...
static void gst_native_init (JNIEnv* env, jobject thiz, jstring rawPipeline) {
  CustomData *data = g_new0 (CustomData, 1);
  SET_CUSTOM_DATA (env, thiz, custom_data_field_id, data);
  GST_DEBUG_CATEGORY_INIT (debug_category, "gst_player", 0, "Android tutorial 3");
  gst_debug_set_threshold_for_name("gst_player", GST_LEVEL_DEBUG);
  GST_DEBUG ("Created CustomData at %p", data);
  data->app = (*env)->NewGlobalRef (env, thiz);
  GST_DEBUG ("Created GlobalRef for app object at %p", data->app);
  const char *nativeString = (*env)->GetStringUTFChars(env, rawPipeline, 0);

  data->rawPipeline = (char*)nativeString;
...
static jboolean gst_native_class_init (JNIEnv* env, jclass klass) {
  ...
  on_end_of_stream_method_id = (*env)->GetMethodID (env, klass, "onStreamEnded", "()V");
  on_error_method_id = (*env)->GetMethodID (env, klass, "onError", "(Ljava/lang/String;)V");
  if (!custom_data_field_id || !set_message_method_id || !on_gstreamer_initialized_method_id) {
    /* We emit this message through the Android log instead of the GStreamer log because the later
     * has not been initialized yet.
     */
    __android_log_print (ANDROID_LOG_ERROR, "gst_player", "The calling class does not implement all necessary interface methods");
    return JNI_FALSE;
  }
  return JNI_TRUE;
}
